...
class AuctionSchedulerImplTest {
    ...
    void testStartAuctions() {
        ...
        Auction auction1 = new Auction(1, 101, now.minusDays(2), now.plusDays(1), 50.0, 75.0, 5.0, AuctionStatus.UPCOMING);
        Auction auction2 = new Auction(2, 102, now.minusHours(1), now.plusHours(1), 100.0, 150.0, 10.0, AuctionStatus.UPCOMING);
	...
	void testEndAuctions() {
        // Arrange
        LocalDateTime now = LocalDateTime.now();
        Auction auction1 = new Auction(1, 101, now.minusDays(2), now.minusDays(1), 50.0, 75.0, 5.0, AuctionStatus.LIVE);
        Auction auction2 = new Auction(2, 102, now.minusDays(1), now.minusHours(1), 100.0, 150.0, 10.0, AuctionStatus.LIVE);
